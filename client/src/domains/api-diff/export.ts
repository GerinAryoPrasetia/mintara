...
import type { CompareRequest, CompareResult, DiffNode, BulkItemResult, BulkRequestItem, NormalizationOptions } from '@mintara/shared'
...
export async function exportBulkExcel(
  bulkResults: BulkItemResult[],
  fallbackNormalization: NormalizationOptions,
  ...
  for (const entry of bulkResults) {
    ...
    const nodes = diffTargets(targets[0].body, targets[1].body, entry.item.normalization ?? fallbackNormalization)
